...
function customify_wc_reset_default_hooks()
{
	...
	if (!has_action('customify/wc-product/before-media', 'woocommerce_template_loop_product_link_open')) {
		add_action('customify/wc-product/before-media', 'woocommerce_template_loop_product_link_open', 10);
	}

	if (!has_action('customify/wc-product/after-media')) {
 ...
 * @since 0.3.9
 */
add_action('customify_wc_loop_start', 'customify_wc_reset_default_hooks');
...
{
?>
	<h2 class="order_review_heading"><?php _e('HÓA ĐƠN', 'customify'); ?></h2>
<?php
}
